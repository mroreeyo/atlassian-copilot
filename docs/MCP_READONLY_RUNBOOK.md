...
The browser only submits the values to the Broker. It does not store tokens in frontend env, localStorage, Zustand, or TanStack Query. The Broker stores the personal profile in server-side local state under `~/.atlassian-copilot/broker` by default, or `AKC_BROKER_STATE_DIR` when set and never returns the token in status responses.
